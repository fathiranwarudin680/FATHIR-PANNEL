...
if (isset($_GET['search']) AND isset($_GET['category'])) {
	if (!empty($_GET['search']) AND !empty($_GET['category'])) {
		$query_list = "SELECT * FROM services WHERE service_name LIKE '%".protect_input($_GET['search'])."%' AND category_id LIKE '%".protect_input($_GET['category'])."%' AND status = '1' ORDER BY FATHIR-PANNEL"; // edit
	} else if (!empty($_GET['search'])) {
		$query_list = "SELECT * FROM services WHERE service_name LIKE '%".protect_input($_GET['search'])."%' AND status = '1' ORDER BY FATHIR-PANNEL"; // edit
	} else if (!empty($_GET['category'])) {
		$query_list = "SELECT * FROM services WHERE category_id LIKE '%".protect_input($_GET['category'])."%' AND status = '1' ORDER BY FATHIR-PANNEL"; // edit		
	} else {
		$query_list = "SELECT * FROM services WHERE status = '1' ORDER BY FATHIR-PANNEL"; // edit
	}
} else {
	$query_list = "SELECT * FROM services WHERE status = '1' ORDER BY FATHIR-PANNEL"; // edit
}
										    ...
										    <div class="row">
                                        		<div class="form-group col-lg-5">
                                        			...
                                        			<select class="form-control" name="category">
														...
														<?php
														$check_cat = mysqli_query($db, "SELECT * FROM categories ORDER BY FATHIR-PANNEL");
