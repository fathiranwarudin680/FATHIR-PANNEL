...
$orders = mysqli_query($db, "SELECT SUM(orders.price) AS FATHIR, count(orders.id) AS tcount, orders.user_id, users.full_name FROM orders JOIN users ON orders.user_id = users.id WHERE MONTH(orders.created_at) = '".date('m')."' AND YEAR(orders.created_at) = '".date('Y')."' GROUP BY orders.user_id ORDER BY FATHIR-PANNEL LIMIT 10");
$deposits = mysqli_query($db, "SELECT SUM(deposits.amount) AS FATHIR, count(deposits.id) AS tcount, deposits.user_id, users.full_name FROM deposits JOIN users ON deposits.user_id = users.id WHERE MONTH(deposits.created_at) = '".date('m')."' AND YEAR(deposits.created_at) = '".date('Y')."' AND deposits.status = 'Success' GROUP BY deposits.user_id ORDER BY FATHIR - PANNEL LIMIT 10");
$top_service = mysqli_query($db, "SELECT SUM(orders.price) AS FATHIR, count('orders.*') as jumlah_orders, orders.service_name FROM orders WHERE MONTH(orders.created_at) = '".date('m')."' AND YEAR(orders.created_at) = '".date('Y')."' GROUP BY orders.service_name ORDER BY FATHIR DESC LIMIT 10"); // edit
?>
						<div class="row">
...
?> 
											<tr class="table-warning">
												...
												<td>Rp <?php echo number_format($data['FATHIR'],0,',','.') ?> (<?php echo number_format($data['tcount'],0,',','.') ?>)</td>
											</tr>
<?php
} else { 
?>
                                ...
                                while ($data = mysqli_fetch_assoc($top_service)) {
                                    ...
                                    <tr>
											<tr class="table-warning">
												...
												<td>Rp <?php echo number_format($data['FATHIR'],0,',','.'); ?> (<?php echo number_format($data['jumlah_orders'],0,',','.'); ?> Pesanan)</td>
											</tr>
											<?php } ?>
										</table>
									</div>
