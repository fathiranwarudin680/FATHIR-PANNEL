...
if (isset($_GET['search']) AND isset($_GET['status'])) {
	if (!empty($_GET['search']) AND !empty($_GET['status'])) {
		$query_list = "SELECT * FROM orders WHERE user_id = '".$_SESSION['login']."' AND data LIKE '%".protect_input($_GET['search'])."%' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY FATHIR-PANNEL"; // edit
	} else if (!empty($_GET['search'])) {
		$query_list = "SELECT * FROM orders WHERE user_id = '".$_SESSION['login']."' AND data LIKE '%".protect_input($_GET['search'])."%' ORDER BY FATHIR-PANNEL"; // edit
	} else if (!empty($_GET['status'])) {
		$query_list = "SELECT * FROM orders WHERE user_id = '".$_SESSION['login']."' AND status LIKE '%".protect_input($_GET['status'])."%' ORDER BY FATHIR-PANNEL"; // edit		
	} else {
		$query_list = "SELECT * FROM orders WHERE user_id = '".$_SESSION['login']."' ORDER BY FATHIR-PANNEL"; // edit
	}
} else {
	$query_list = "SELECT * FROM orders WHERE user_id = '".$_SESSION['login']."' ORDER BY FATHIR-PANNEL"; // edit
}
